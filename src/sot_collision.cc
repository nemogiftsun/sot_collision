...
SotCollision::SotCollision(const std::string &inName) : Entity(inName),
              ...
              "sotCollision(" + name + ")::output(vector)::" + std::string("closestPointj"))
{
  ...
  addCommand(std::string("getcheck"),
             new ::dynamicgraph::command::Getter<SotCollision, bool>(*this, &SotCollision::getcheck, docstring));
  docstring =
      "\n"
      "    Get dimension of the collision model\n"
      "\n";
  addCommand(std::string("getdimension"),
             new ::dynamicgraph::command::Getter<SotCollision, double>(*this, &SotCollision::getdimension, docstring));

  addCommand(std::string("getfclstate"),
             new ::dynamicgraph::command::Getter<SotCollision, dg::Matrix>(*this, &SotCollision::getfclstate, docstring));
      ...
      else if (link_info_map[i].link_type == "box")
      {
        ...
        else if (link_info_map[j].link_type == "box")
        {
          ...
          if (link_info_map[j].link_location == "external")
          {
            ...
            Transform3f tli = transform_links[i];
            Transform3f tlj = transform_links[i];
...
dg::Matrix &SotCollision::computeimdJacobian(Matrix &res, int time)
{
  ...
  const int cols = (body_jacobian_input[0]->access(time)).cols();
  res.resize(1, cols);
  ...
  for (int p = 0; p < num_collisionpairs; p++)
  {
    ...
    closestPointI(0, p) = l1[0];
    closestPointI(1, p) = l1[1];
    closestPointI(2, p) = l1[2];
    closestPointJ(0, p) = l2[0];
    closestPointJ(1, p) = l2[1];
    closestPointJ(2, p) = l2[2];
    ...
    aclosestpointtransform(0, 3) = l1[0];
    aclosestpointtransform(1, 3) = l1[1];
    aclosestpointtransform(2, 3) = l1[2];
    //aclosestpointtransform(0,3) = 0;  aclosestpointtransform(1,3) =0.045 ; aclosestpointtransform(2,3)=0;
    bclosestpointtransform(0, 3) = l2[0];
    bclosestpointtransform(1, 3) = l2[1];
    bclosestpointtransform(2, 3) = l2[2];
    ...
    ssmaclosestpoint(0, 0) = 0;
    ssmaclosestpoint(0, 1) = -aclosestpointlocalframe(2, 3);
    ssmaclosestpoint(0, 2) = aclosestpointlocalframe(1, 3);
    ssmaclosestpoint(1, 0) = aclosestpointlocalframe(2, 3);
    ssmaclosestpoint(1, 1) = 0;
    ssmaclosestpoint(1, 2) = -aclosestpointlocalframe(0, 3);
    ssmaclosestpoint(2, 0) = -aclosestpointlocalframe(1, 3);
    ssmaclosestpoint(2, 1) = aclosestpointlocalframe(0, 3);
    ssmaclosestpoint(2, 2) = 0;
    /*
    ...
    ssmbclosestpoint(0, 0) = 0;
    ssmbclosestpoint(0, 1) = -bclosestpointlocalframe(2, 3);
    ssmbclosestpoint(0, 2) = bclosestpointlocalframe(1, 3);
    ssmbclosestpoint(1, 0) = bclosestpointlocalframe(2, 3);
    ssmbclosestpoint(1, 1) = 0;
    ssmbclosestpoint(1, 2) = -bclosestpointlocalframe(0, 3);
    ssmbclosestpoint(2, 0) = -bclosestpointlocalframe(1, 3);
    ssmbclosestpoint(2, 1) = bclosestpointlocalframe(0, 3);
    ssmbclosestpoint(2, 2) = 0;
    ...
    ln.normalize();
    //std::cout <<"normalized = " <<ln<<std::endl;
...
dynamicgraph::SignalPtr<MatrixHomogeneous, int> &SotCollision::createPositionSignalIN(const std::string &signame)
{
  ...
  dynamicgraph::SignalPtr<MatrixHomogeneous, int> *sig = new dynamicgraph::SignalPtr<MatrixHomogeneous, int>(NULL, "sotCollision(" + name + ")::input(matrix)::" + signame);
...
dynamicgraph::SignalPtr<dg::Matrix, int> &SotCollision::createJacobiansignalIN(const std::string &signame)
{
  ...
  dynamicgraph::SignalPtr<dg::Matrix, int> *sig = new dynamicgraph::SignalPtr<dg::Matrix, int>(NULL, "sotCollision(" + name + ")::input(matrix)::" + signame);
